...
DoesItMove::DoesItMove() noexcept:
  m_ObjectDirectionMutex{},
...
void DoesItMove::setObjectDirection(opendlv::proxy::ObectDirectionRequest const &ObjectDirection) noexcept
{
  std::lock_guard<std::mutex> lock(m_ObjectDirectionMutex);
  m_ObjectDirection = ObjectionDirection.ObjectDirection();
}



void DoesItMove::setGroundSteeringAngle(opendlv::proxy::GroundSteeringRequest const &groundSteeringAngle) noexcept
{
  std::lock_guard<std::mutex> lock(m_groundSteeringAngleMutex);
  m_groundSteeringAngle = groundSteeringAngle.groundSteering();
}

void DoesItMove::setPedalPosition(opendlv::proxy::PedalPositionRequest const &pedalPosition) noexcept
{
  std::lock_guard<std::mutex> lock(m_pedalPositionMutex);
  m_pedalPosition = pedalPosition.position();
}

opendlv::sim::KinematicState DoesItMove::step(double dt) noexcept
{
